...
import os
FILEPATH_DOMAINS = os.path.join(os.path.dirname(__file__), "..", "domains", "ctRSD_domains_list_210.xls")
FILEPATH_OUTPUTS = os.path.join(os.path.dirname(__file__), "..", "outputs")
...
def get_complement(seq):
    ...
    new_seq = ""
    for element in seq:
        new_seq += complement_map[element]
...
def search_xldict(sheet_dict, key):
    # Helper function for ctRSD_seq_compile()
...
def create_template(ctRSD_part,prom_S,rna,us_S,ds_S,temp_len):
    ...
    # RNA Template
    rna_template = rc_seq(ctRSD_part, spec_out='s',rna=1)[1:-1]
        
    return template, rna_template
...
def write_genbank(sequence, output_path=None, locus_name="ctRSD_sequence", annotations=None, rna=False):
    ...
    with open(output_path, "w", encoding="utf-8") as fh:
        ...
        for start in range(0, len(seq), 60):
            ...
            fh.write(f"{start+1:>9} ")
            fh.write(" ".join(chunk[i:i+10] for i in range(0, len(chunk), 10)))
            fh.write("\n")
        fh.write("//\n")
...
class RSD_sim:
    ...
    def ctRSD_seq_compile(self,name,filepath=FILEPATH_DOMAINS,Rz='Ro',L='L',term='T7t',hp5='5hp',prom='T7p',\
        ...
        th_d = xlsheet_to_dict(filepath, 'th', 1)  # toehold sequences (th')
